...
class Module extends \humhub\components\Module
{
    ...
    {
        if(Yii::$app->controller->module && Yii::$app->controller->module->id == 'emailwhitelist' 
    ...
    public static function onAuthenticationMenuInit($event)
    {
        $event->sender->addItem([
            ...
            'sortOrder' => 300,
            'isActive' => (Yii::$app->controller->module && Yii::$app->controller->module->id == 'emailwhitelist' 
                    && Yii::$app->controller->id == 'admin' && Yii::$app->controller->action->id == 'index'),
        ]);
    }
    ...
    {      
        ...
        if(!empty($disallowedEmails)) {
            $event->sender->addError('emails', 
                    Yii::t('EnterpriseModule.emailwhitelist', 'The follwing emails do not match the whitelist settings: {emails}', ['emails' => implode(", ", $disallowedEmails)]));
            $event->sender->addError('inviteExternal', 
                    Yii::t('EnterpriseModule.emailwhitelist', 'The follwing emails do not match the whitelist settings: {emails}', ['emails' => implode(", ", $disallowedEmails)]));
